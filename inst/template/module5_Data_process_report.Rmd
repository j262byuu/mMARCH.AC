...
setwd(currentdir)  
# Use openxlsx instead of xlsx. Java is no longer required so no more java.lang.OutOfMemoryError.
library(openxlsx)  
library(knitr)
...
# Changed to openxlsx::read.xlsx
D2<-openxlsx::read.xlsx(summaryFN1,startRow=1,sheet = sheetName[2])  
...
# Changed to openxlsx::read.xlsx
Device<-openxlsx::read.xlsx(summaryFN1,startRow=1,sheet = "10_deviceInfo") 
...
# Changed to openxlsx::read.xlsx
D3<-openxlsx::read.xlsx(summaryFN1,startRow=1,sheet = sheetName[3])  
if (ncol(D3)>=2) { 
D3.uids<-unique(D3[,2])
...
# Changed to openxlsx::read.xlsx
D9<-openxlsx::read.xlsx(summaryFN1,startRow=1,sheet = sheetName[9])
#dim(D9) ;head(D9)  
par(mfrow=c(2,2))
...
summaryFN1<-paste(currentdir,"/summary/",studyname,"_ggir_output_summary.xlsx",sep="")
# Changed to openxlsx::read.xlsx
summaryP2<-openxlsx::read.xlsx(summaryFN1,startRow=1,sheet = "2_fileSummary") 
# The write function in openxlsx does not insert an 'X' in Row 1, Column 1, so gsub is no longer needed.
ggirV<-as.character(colnames(summaryP2)[1])  
